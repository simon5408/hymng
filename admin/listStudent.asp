	...
	<script language="javascript" type="text/javascript">
		...
		function doDel(id){
			...
			if(confirm("确定要删除该学生信息？")){
				window.location.href="delStudent.asp?delType=0&studentId="+id+"&addr="+addr;
			}
		}
		...
		function doSearch(){
			...
			var ticheng = document.getElementById("ticheng").value;
			...
			document.location.href="listStudent.asp?stuName="+stuName+"&ticheng="+ticheng+"&smobile="+smobile+"&usersId="+usersId+"&subUserId="+subUserId+"&comId="+comId+"&teacher="+teacher+"&inYear="+inYear+"&schId="+schId+"&eduId="+eduId+"&isDegree="+isDegree+"&isZbtao="+isZbtao+"&arrange="+arrange+"&specId="+specId+"&centerId="+centerId+"&stuFeeYear="+stuFeeYear+"&stuFeeMonth="+stuFeeMonth+"&fromYear="+fromYear+"&toYear="+toYear+"&fromFeeYear="+fromFeeYear+"&toFeeYear="+toFeeYear+"&fromTypeFeeYear="+fromTypeFeeYear+"&toTypeFeeYear="+toTypeFeeYear;
		}
		
		// 欠费查询
		function doLackFeeSearch(){
			window.open("lackFeeMng.asp","","toolbar=no,scrollbars=yes,directories=no,location=no,status=no,menubar=yes,resizable=no,fullscreen=yes");
		}
		
		// 交费查询
		function doChargeFeeSearch(){
			window.open("chargeFeeMng.asp","","toolbar=no,scrollbars=yes,directories=no,location=no,status=no,menubar=yes,resizable=no,fullscreen=yes");
		}
		...
		function doBatchDel() {
			...
			if (delIds != "") {
				if(confirm("确定要删除这些学生信息？")){
					window.location.href="delStudent.asp?delType=0&studentId="+delIds+"&addr="+addr;
				}
			}else{
				alert("请选择学生信息！");
				return false;
...
<% username=session("username") 
	if username<>"" then 
	session("typeName") = "在读学生信息管理"
	session("leftZero") = "1"
	userId = session("user_id")
	
%>
<!--#include file="listStudentAct.asp" -->
	<!-- 头部操作菜单栏 -->
	

	
	<!-- 主体功能区(开始) -->
	<div id="searDiv" style='margin-top:0px;'>
	<table class="trgTbl" width="100%" border="0" cellpadding="1" cellspacing="1" style="background:#f5f5f5">
		<tr>
			...
			<td style="font-family:'幼圆', Helvetica, sans-serif;">介绍人：</td>
			<td><input type="text" name="ticheng" id="ticheng" value="<%=session("ticheng")%>"></td>
		...
		<tr>
			...
			<td colspan="2">
				收费日期：
				<input name="fromFeeYear" id="fromFeeYear" type="text" style="width:100px;" size="12" value="<%=session("fromFeeYear")%>" class="Wdate" onfocus="WdatePicker({isShowClear:false,readOnly:false})" /> - 
				<input name="toFeeYear" id="toFeeYear" type="text" size="12" style="width:100px;" value="<%=session("toFeeYear")%>" class="Wdate" onfocus="WdatePicker({isShowClear:false,readOnly:false})" />
			</td>
	...
	<div class="infoDiv" style="margin-left:8px;">
		...
		<a href="javascript:void(0);" onclick="doLackFeeSearch()" class="easyui-linkbutton" icon="icon-search">未交费查询</a>
		<a href="javascript:void(0);" onclick="doChargeFeeSearch()" class="easyui-linkbutton" icon="icon-search">已交费查询</a>
	...
	<table width="100%" border="0" cellpadding="1" cellspacing="1" style="background:#a0a0a0; font-family:'微软雅黑', Helvetica, sans-serif;">
		...
		<%
			...
			fee8 = 0
			if(IsNull(rs("fee8"))) then
				fee8 = 0
			else
				fee8 = rs("fee8")
			end if
			
			allFee = rs("fee1")+rs("fee2")+rs("fee3")+rs("fee4")+rs("fee5")+rs("fee6")+fee7+fee8-rs("total_refund_fee")
		...
		<tr>
			<td><%=i%></td>
			<td><input type="checkbox" value="<%=rs("stu_id")%>" onclick="doCheckChk()" <% if userId <> 1 then %>disabled<% end if %> /></td>
			<td><a href='editStudent.asp?studentId=<%=rs("stu_id")%>' target='_blank' title="备注：&#13;&#10;<%=replace(rs("smemo"),"\u0028","+")%>&#13;&#10;---------&#13;&#10;学生说明：&#13;&#10;<%=rs("stu_info")%>" style="<% if rs("watch_status")=0 and userId=1 then %>color:red;<%else%>color:black; <% end if %>font-weight:bold;"><%=rs("stu_name")%></a></td>
			<td><%=rs("smobile")%>&nbsp;</td>
			<td><%=rs("username")%></td>
			<%
				subUsername = 0
			...
			<%
				ticheng = "-"
				if(IsNull(rs("ticheng")) or rs("ticheng") = "" or rs("ticheng") = "0") then
					ticheng = "-"
				else
					ticheng = rs("ticheng")
				end if
			...
			<td><%=ticheng %></td>
			<% 	typeFee1Years = "-"
				if(rs("type_fee1_year") <> "1970-01-01" and rs("type_fee1_year") <> "1970-1-1" and rs("type_fee1_year") <> "1/1/1970") then 
				  typeFee1Years = rs("type_fee1_year")
				end if
				typeFee2Years = "-"
				if(rs("type_fee2_year") <> "1970-01-01" and rs("type_fee2_year") <> "1970-1-1" and rs("type_fee2_year") <> "1/1/1970") then 
				  typeFee2Years = rs("type_fee2_year")
				end if
				typeFee3Years = "-"
				if(rs("type_fee3_year") <> "1970-01-01" and rs("type_fee3_year") <> "1970-1-1" and rs("type_fee3_year") <> "1/1/1970") then 
				  typeFee3Years = rs("type_fee3_year")
				end if
				typeFee4Years = "-"
				if(rs("type_fee4_year") <> "1970-01-01" and rs("type_fee4_year") <> "1970-1-1" and rs("type_fee4_year") <> "1/1/1970") then 
				  typeFee4Years = rs("type_fee4_year")
				end if
				%>
			<td title="总返利额：￥<%=totalTypeFee%>;/已返利额：￥<%=typeFee%>;&#13;&#10;具体返利：&#13;&#10;1，费用：<%=rs("type_fee1")%>元(日期：<%=typeFee1Years%>);&#13;&#10;2，费用：<%=rs("type_fee2")%>元(日期：<%=typeFee2Years%>);&#13;&#10;3，费用：<%=rs("type_fee3")%>元(日期：<%=typeFee3Years%>);&#13;&#10;4，费用：<%=rs("type_fee4")%>元(日期：<%=typeFee4Years%>);">
				<% if rs("teacher")<>"" then  %>
					<%=rs("teacher")%>
				<% else %>
					-
			...
			<td><%=rs("sch_center")%></td>
			<!--  title="[应收总费用]/[已收总费用]&#13;&#10;[交予学校费用]/[利润]" -->
			<td id="<%=rs("stu_id")%>" class="showCur">
				￥<%=totalFee%>/￥<%=allFee%><br />
				￥<%=ftotalFee%>/￥<%=gain%>
				
				<div class="subDiv" id="feeCot<%=rs("stu_id")%>" style=" font-family:'幼圆', Helvetica, sans-serif;">
				<div style="height:20px; background:#e0e0e0">
					<b>总费用：</b> <%=rs("total_fee")%>元
				</div>
				
                <% 	fee1Years = "-"
                    if(rs("fee1_year") <> "1970-01-01" and rs("fee1_year") <> "1970-1-1" and rs("fee1_year") <> "1/1/1970") then 
                      fee1Years = rs("fee1_year")
                    end if
                    fee2Years = "-"
                    if(rs("fee2_year") <> "1970-01-01" and rs("fee2_year") <> "1970-1-1" and rs("fee2_year") <> "1/1/1970") then 
                      fee2Years = rs("fee2_year")
                    end if
                    fee3Years = "-"
                    if(rs("fee3_year") <> "1970-01-01" and rs("fee3_year") <> "1970-1-1" and rs("fee3_year") <> "1/1/1970") then 
                      fee3Years = rs("fee3_year")
                    end if
                    fee4Years = "-"
                    if(rs("fee4_year") <> "1970-01-01" and rs("fee4_year") <> "1970-1-1" and rs("fee4_year") <> "1/1/1970") then 
                      fee4Years = rs("fee4_year")
                    end if
                    fee5Years = "-"
                    if(rs("fee5_year") <> "1970-01-01" and rs("fee5_year") <> "1970-1-1" and rs("fee5_year") <> "1/1/1970") then 
                      fee5Years = rs("fee5_year")
                    end if
					fee6Years = "-"
                    if(rs("fee6_year") <> "1970-01-01" and rs("fee6_year") <> "1970-1-1" and rs("fee6_year") <> "1/1/1970") then 
                      fee6Years = rs("fee6_year")
                    end if
					fee7Years = "-"
                    if(rs("fee7_year") <> "1970-01-01" and rs("fee7_year") <> "1970-1-1" and rs("fee7_year") <> "1/1/1970") then 
                      fee7Years = rs("fee7_year")
                    end if
					fee8Years = "-"
                    if(rs("fee8_year") <> "1970-01-01" and rs("fee8_year") <> "1970-1-1" and rs("fee8_year") <> "1/1/1970") then 
                ...
                <div>
                 ...
                 <% 	
				  	refundFee1Years = "-"
                    if(rs("refund_fee1_year") <> "1970-01-01" and rs("refund_fee1_year") <> "1970-1-1" and rs("refund_fee1_year") <> "1/1/1970") then 
                      refundFee1Years = rs("refund_fee1_year")
                    end if
                    refundFee2Years = "-"
                    if(rs("refund_fee2_year") <> "1970-01-01" and rs("refund_fee2_year") <> "1970-1-1" and rs("refund_fee2_year") <> "1/1/1970") then 
			...
			<td style="width:80px;">
				<a href='editStudent.asp?studentId=<%=rs("stu_id")%>' target='_blank' class="easyui-linkbutton" plain="true">编辑</a>
				<a <% if username="admin" then %>href='javascript:doDel(<%=rs("stu_id")%>)'<% end if%> class="easyui-linkbutton" plain="true">删除</a>
			</td>
		</tr>
		<% 
			rs.movenext
		...
		<tr>
			...
			<td  colspan="11" style="background:#dfdfdf"></td>
