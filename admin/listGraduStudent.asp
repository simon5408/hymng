	...
	<script language="javascript" type="text/javascript">
		...
		function doDel(id){
			...
			if(confirm("确定要删除该学生信息？")){
				window.location.href="delStudent.asp?delType=1&studentId="+id+"&addr="+addr;
			}
		}
		...
		function doBatchDel() {
			...
			if (delIds != "") {
				if(confirm("确定要删除这些学生信息？")){
					window.location.href="delStudent.asp?delType=1&studentId="+delIds+"&addr="+addr;
				}
			}else{
				alert("请选择学生信息！");
				return false;
...
<% username=session("username") 
	if username<>"" then 
	session("typeName") = "毕业学生信息管理"
	session("leftZero") = "1"
	
%>
<!--#include file="listGraduStudentAct.asp" -->
	<!-- 头部操作菜单栏 -->
	...
	<table class="trgTbl" width="100%" border="0" cellpadding="1" cellspacing="1" style="background:#f5f5f5">
		...
		<tr>
			...
			<td colspan="2">
				...
				<input name="toFeeYear" id="toFeeYear" type="text" style="width:100px;" size="12" value="<%=session("toFeeYear")%>" class="Wdate" onfocus="WdatePicker({isShowClear:false,readOnly:false})" />
			</td>
...
<div class="infoDiv" style="margin-left:8px;">
	...
	<table width="100%" border="0" cellpadding="1" cellspacing="1" style="background:#a0a0a0; font-family:'微软雅黑', Helvetica, sans-serif;">
		...
		<%
			...
			jsr = rs("ticheng")
			if (IsNull(jsr) or jsr = "0") then
				jsr = "-"
			end if
		%>
		<tr>
			<td><%=i%></td>
			<td><input type="checkbox" value="<%=rs("gdu_stu_id")%>" onclick="doCheckChk()" <% if userId <> 1 then %>disabled<% end if %> /></td>
			<td><a href='editGraduStudent.asp?gduStuId=<%=rs("gdu_stu_id")%>' target='_blank' title="备注：&#13;&#10;<%=replace(rs("smemo"),"\u0028","+")%>&#13;&#10;---------&#13;&#10;学生说明：&#13;&#10;<%=rs("stu_info")%>" style="color:#000000;"><%=rs("stu_name")%></a></td>
			<td><%=jsr%>&nbsp;</td>
			<td><%=rs("smobile")%>&nbsp;</td>
			<%
				gduMemo = ""
			...
			<td><%=rs("sch_center")%></td>
			<!--  title="[应收总费用]/[已收总费用]&#13;&#10;[交予学校费用]/[利润]" -->
			<td id="<%=rs("gdu_stu_id")%>" class="showCur">
				￥<%=totalFee%>/￥<%=allFee%><br />
				￥<%=ftotalFee%>/￥<%=gain%>
				
				<div class="subDiv" id="feeCot<%=rs("gdu_stu_id")%>" style=" font-family:'幼圆', Helvetica, sans-serif;">
                ...
                <div>
					...
					1,<%=rs("fee1")%>元(日期：<%=fee1Years%><br />备注：<%=rs("fee1_memo")%>)<br />
				  	...
				  	7,<%=rs("fee7")%>元(日期：<%=fee7Years%><br />备注：<%=fee7Memo%>)<br />
				  	8,<%=rs("fee8")%>元(日期：<%=fee8Years%><br />备注：<%=fee8Memo%>)
				</div>
                <div style="height:20px; background:#e0e0e0">
			...
			<td style="width:80px;">
				<a href='editGraduStudent.asp?gduStuId=<%=rs("gdu_stu_id")%>' target='_blank' class="easyui-linkbutton" plain="true">编辑</a> 
				<a <% if username="admin" then %>href='javascript:doDel(<%=rs("gdu_stu_id")%>)'<% end if%> class="easyui-linkbutton" plain="true">删除</a>
			</td>
		</tr>
		<% 
			rs.movenext
		...
		%>
		<tr>
			<td colspan="9" style="background:#dfdfdf"></td>
